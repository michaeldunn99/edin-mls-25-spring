...
def our_kmeans_L2_CUPY_updated(N, D, A, K, scaling_factor=1, num_streams=2, max_iters=10, profile_mem=False):
    tol = 1e-4
    gpu_batch_size, gpu_batch_num = optimum_k_means_batch_size(N, D, K, "l2" , scaling_factor, num_streams)
    print(f"Scale factor is {scaling_factor}")
    ...
    for j in range(max_iters):
        ...
        for i, (start, end) in enumerate(gpu_batches):
            print(f"Processing batch {i} with start {start} and end {end} in stream {i%num_streams}")
            ...
            with stream:
                ...
                cp.matmul(A_batch, centroids_gpu.T, out=dot_matrix[i % num_streams][:batch_size])
                
                dot = dot_matrix[i % num_streams][:batch_size]
                #alt: dot = A_batch @ centroids_gpu.T
                ...
                batch_counts = cp.sum(one_hot, axis=0, dtype=cp.int32)  # (K,)
...
def our_kmeans_L2_CUPY_updated_profiled(N, D, A, K, scaling_factor=1, num_streams=2, max_iters=10, profile=False, A_threshold_GB=8.0):
...
# def our_kmeans_L2_CUPY_updated_profiled(N, D, A, K, scale_factor=1, num_streams=1, max_iterations=10):
#     timings = defaultdict(list)
#     tol = 1e-4
#     gpu_batch_size, gpu_batch_num = optimum_k_means_batch_size(N, D, K, "l2", scale_factor, num_streams)
#     print(f"gpu_batch_size is {gpu_batch_size}")
#     print(f"gpu_batch_num is {gpu_batch_num}")
#     print(f"Expected memory jus from data is {gpu_batch_size * num_streams * D * 4 / (1024**2)} MB")
...
#     centroids_gpu = time_cpu_section("Copy initial centroids to GPU", lambda: cp.asarray(A[indices], dtype=cp.float32), timings)

#     streams = [cp.cuda.Stream(non_blocking=True) for _ in range(num_streams)]
#     print_mem("Before dot product matrix")
#     dot_matrix = [cp.empty((gpu_batch_size, K), dtype=cp.float32) for _ in range(num_streams)]
#     print_mem("After dot product matrix, before A_device")
#     print_mem("Before copy to GPU")
#     A_device = [cp.empty((gpu_batch_size, D), dtype=cp.float32) for _ in range(num_streams)]
#     print_mem("After copy to GPU")
...
#             # with stream:
#             time_gpu_section(f"Copy A_buf", stream, lambda: A_buf[:batch_size].set(A[start:end]), timings)
#             A_batch = A_buf[:batch_size]

#             A_norm = time_gpu_section("A_norm", stream, lambda: cp.sum(A_batch ** 2, axis=1, keepdims=True), timings)
#             C_norm = time_gpu_section("C_norm", stream, lambda: cp.sum(centroids_gpu ** 2, axis=1, keepdims=True).T, timings)
#             dot = time_gpu_section("Dot", stream, lambda: cp.matmul(A_batch, centroids_gpu.T, out=dot_matrix[i % num_streams][:batch_size]), timings)
#             distances = time_gpu_section("Distance matrix calc", stream, lambda: A_norm + C_norm - 2 * dot, timings)

#             assignments = time_gpu_section("Argmin", stream, lambda: cp.argmin(distances, axis=1), timings)
#             assignments_buf[:batch_size] = assignments
#             cluster_assignments[start:end] = assignments_buf[:batch_size]

#             one_hot = time_gpu_section("One-hot", stream, lambda: cp.eye(K, dtype=A_batch.dtype)[assignments], timings)
#             batch_cluster_sum = time_gpu_section("Cluster sum", stream, lambda: one_hot.T @ A_batch, timings)
#             batch_counts = time_gpu_section("Cluster counts via one-hot", stream, lambda: cp.sum(one_hot, axis=0, dtype=cp.int32), timings)
#             # batch_counts = time_gpu_section("Bincount", stream, lambda: cp.bincount(assignments, minlength=K), timings)

#             cluster_sums_stream[i % num_streams] += batch_cluster_sum
#             counts_stream[i % num_streams] += batch_counts
                
#         #free up memory via free all blocks
        
#         cp.cuda.Device().synchronize()

#         cluster_sum = time_gpu_section_default("Sum cluster_sums_stream", lambda: sum(cluster_sums_stream), timings)
#         counts = time_gpu_section_default("Sum counts_stream", lambda: sum(counts_stream), timings)

#         dead_mask = time_gpu_section_default("Dead mask calc", lambda: (counts == 0), timings)
#         counts = time_gpu_section_default("Clamp counts", lambda: cp.maximum(counts, 1), timings)
#         updated_centroids = time_gpu_section_default("Update centroids", lambda: cluster_sum / counts[:, None], timings)
...
#         # mem_before = get_gpu_memory()
#         # start = time.perf_counter()
#         # del A_norm, C_norm, dot, distances, cluster_sums_stream, counts_stream, A_buf, assignments_buf
#         # if 'assignments' in locals(): del assignments
#         # if 'one_hot' in locals(): del one_hot
#         # if 'batch_cluster_sum' in locals(): del batch_cluster_sum
#         # if 'batch_counts' in locals(): del batch_counts
...
#         # elapsed = (time.perf_counter() - start) * 1000
#         # mem_after = get_gpu_memory()
#         # print(f"Cleaned GPU memory in {elapsed:.3f} ms, memory: {mem_before} -> {mem_after} MB")
#         # cp.cuda.Device().synchronize()
#         # timings["Clean GPU memory"].append((elapsed, mem_before, mem_after))
#         # #print a line of starts
#         # print("*" * 80)
#         # print(f"End of iteration {j}\n\n\n\n")
#         # print("*" * 80)
#         # print(f"\n\n\n\n")
...
#         times = [t[0] for t in records]
#         print(f"{name}: mean {np.mean(times):.2f} ms, std {np.std(times):.2f} ms")

#     return cp.asnumpy(cluster_assignments), cp.asnumpy(centroids_gpu)

# def our_kmeans_L2_CUPY_updated_profiled_one_stream(N, D, A, K, scale_factor=1, max_iterations=10):
#     timings = defaultdict(list)
#     tol = 1e-4
#     gpu_batch_size, gpu_batch_num = optimum_k_means_batch_size(N, D, K, "l2", scale_factor)
#     print(f"gpu_batch_size is {gpu_batch_size}")
#     print(f"gpu_batch_num is {gpu_batch_num}")
#     print(f"Expected memory jus from data is {gpu_batch_size * D * 4 / (1024**2)} MB")

#     gpu_batches = [(i * gpu_batch_size, min((i + 1) * gpu_batch_size, N)) for i in range(gpu_batch_num)]
#     dot_matrix_regular = cp.empty((gpu_batch_size, K), dtype=cp.float32)
#     last_batch_size = N - (gpu_batch_num - 1) * gpu_batch_size
#     dot_matrix_last = cp.empty((last_batch_size, K), dtype=cp.float32)
...
#     print_mem("After dot product matrix, before A_device")
#     print_mem("Before copy to GPU")
#     A_device = cp.empty((gpu_batch_size, D), dtype=cp.float32)
#     print_mem("After copy to GPU")

#     assignments_gpu = cp.empty(gpu_batch_size, dtype=cp.int32)

#     for j in range(max_iterations):

#         print(f"Max iters is {j}")

#         cluster_sums= cp.zeros((K, D), dtype=cp.float32)
#         counts = cp.zeros(K, dtype=cp.int32)

#         for i, (start, end) in enumerate(gpu_batches):
#             print(f"Processing batch {i} with start {start} and end {end}")
#             print(f"Start is {start}, end is {end}")
#             batch_size = end - start

#             if batch_size == gpu_batch_size:
#                 dot_buf = dot_matrix_regular
#             else:
#                 dot_buf = dot_matrix_last
#             A_buf = A_device

#             assignments_buf = assignments_gpu
            
#             cluster_assignments = cp.empty(N, dtype=np.int32)

#             # with stream:
#             time_gpu_section_default(f"Copy A_buf", lambda: A_buf[:batch_size].set(A[start:end]), timings)
#             A_batch = A_buf[:batch_size]

#             # A_norm = time_gpu_section_default("A_norm", lambda: cp.sum(A_batch ** 2, axis=1, keepdims=True), timings)
#             # C_norm = time_gpu_section_default("C_norm", lambda: cp.sum(centroids_gpu ** 2, axis=1, keepdims=True).T, timings)
#             # dot = time_gpu_section_default(
#             #     "Dot",
#             #     lambda: cp.matmul(A_batch, centroids_gpu.T, out=dot_buf),
#             #     timings
#             # )

            

#             assignments = time_gpu_section_default("Argmin", lambda: cp.argmin(distances, axis=1), timings)
#             assignments_buf[:batch_size] = assignments
#             cluster_assignments[start:end] = assignments_buf[:batch_size]

#             one_hot = time_gpu_section_default("One-hot", lambda: cp.eye(K, dtype=A_batch.dtype)[assignments], timings)
#             batch_cluster_sum = time_gpu_section_default("Cluster sum", lambda: one_hot.T @ A_batch, timings)
#             batch_counts = time_gpu_section_default("Cluster counts via one-hot", lambda: cp.sum(one_hot, axis=0, dtype=cp.int32), timings)

#             cluster_sums += batch_cluster_sum
#             counts += batch_counts

#         dead_mask = time_gpu_section_default("Dead mask calc", lambda: (counts == 0), timings)
#         counts = time_gpu_section_default("Clamp counts", lambda: cp.maximum(counts, 1), timings)
#         updated_centroids = time_gpu_section_default("Update centroids", lambda: cluster_sums / counts[:,None], timings)
...
#         mem_before = get_gpu_memory()
#         start = time.perf_counter()
#         del A_norm, C_norm, dot, distances, A_buf, assignments_buf, dot_buf
#         if 'assignments' in locals(): del assignments
#         if 'one_hot' in locals(): del one_hot
#         if 'batch_cluster_sum' in locals(): del batch_cluster_sum
#         if 'batch_counts' in locals(): del batch_counts
